...
# read all eas polygon 
eas = readOGR('C:/Users/mmann/Desktop/AgSS EAS','EnumerationAreasSIN',stringsAsFactors = F) # R_CODE Z_CODE W_CODE KK_CODE T_CODE
eas$myid = 1:length(eas)
eas_data = eas@data
eas_data$R_CODE = as.numeric(eas_data$R_CODE)
eas_data$Z_CODE = as.numeric(eas_data$Z_CODE)
eas_data$W_CODE = as.numeric(eas_data$W_CODE)
eas_data$KK_CODE = as.numeric(eas_data$KK_CODE)
eas_data$T_CODE = as.numeric(eas_data$T_CODE)

# get coordinates and reproject to EPSG 32637 to match the UTM projection of the original file to create x and y coords
coord = as.data.frame(coordinates(eas))
names(coord) = c('x_coord', 'y_coord')
coordinates(coord) = ~x_coord+y_coord
proj4string(coord) = proj4string(eas)
coord = spTransform(coord, CRS("+init=epsg:32637"))
eas_data$x_coord = coord@coords[,1]
eas_data$y_coord = coord@coords[,2]


# define setting for RSata Package
schooseStataBin() #  C:\\Program Files (x86)\\Stata14\\StataMP-64.exe
options("RStata.StataVersion" = 14)
options("RStata.stata.path" = 'C:/Program Files (x86)/Stata14/StataMP-64/')

source_code = '
* make replacements to data to create matching codes
...
replace EA_NAME="004-02" if EA_NAME=="NA" & RK_NAME=="Lafito Yemenigist Den"
replace EA_NAME="004-01" if EA_NAME=="NA" & RK_NAME=="Jeyi Yemenigist Den"
replace EA_NAME="004-04" if EA_NAME=="NA" & RK_NAME=="Yemenigist Den"
...
replace EA_NAME="020-01" if EA_NAME=="NA" &  RK_NAME=="Sefafi Erisha"
replace EA_NAME="021-01" if EA_NAME=="NA" & RK_NAME=="Alatish Park"
replace EA_NAME="022-01" if EA_NAME=="NA" & RK_NAME=="Not covered by field work"
replace EA_NAME="015-01" if EA_NAME=="NA" & RK_NAME=="Agensera"
replace EA_NAME="016-01" if EA_NAME=="NA" & RK_NAME=="Akule"
replace EA_NAME="014-01" if EA_NAME=="NA" & RK_NAME=="Dibanina wuhin"
...
*THIS IS A PROBLEM, three EAs all labeled the same so I just gave them independent values but may not match up with other data
...
replace EA_NAME="014-06" if EA_NAME=="014-05" & RK_NAME=="Zazi" & x_coord > 145618 & x_coord < 145622
...
*This one is a mess and should be checked with mapping to see its location
...
replace EA_NAME="014-06" if EA_NAME=="014-04" & RK_NAME=="Terche" & x_coord < 455984.2 
...
replace EA_NAME="016-04" if EA_NAME=="016-03" & RK_NAME=="Chuche" & x_coord <  470300 
...
replace EA_NAME="022-03" if EA_NAME=="022-02" & RK_NAME=="ZONIGAWACHA" & x_coord > 342000
...
replace EA_NAME="002-01" if EA_NAME=="NA" & RK_NAME=="UnCovered by Field Work" & W_NAME=="Dima" 
...
replace EA_NAME="001-01" if EA_NAME=="NA" & RK_NAME=="Sekele" 
...
replace EA_NAME="022-01" if EA_NAME=="NA" & RK_NAME=="Jeyi Yemenigist Den"
...
replace EA_NAME="004-02" if EA_NAME=="004-01"& RK_NAME=="Sendabo Dengoro" & x_coord < 309000
...
replace EA_NAME="039-01" if EA_NAME=="NA"& RK_NAME=="Edo Laki Island" & x_coord < 475000
replace EA_NAME="039-02" if EA_NAME=="NA"& RK_NAME=="Edo Laki Island"
replace EA_NAME="010-01" if EA_NAME=="001-01"& RK_NAME=="Ilasa Hagala"
replace EA_NAME="017-01" if EA_NAME=="NA" & RK_NAME=="UnCovered by Field Work"
...
replace EA_NAME="401-01" if EA_NAME=="NA"& RK_NAME=="Sama Yearbtoader Akababi"
replace EA_NAME="401-02" if EA_NAME=="NA"& RK_NAME=="Yemengist Den" & W_NAME=="Anchar"
...
replace T_CODE=1 if   RK_NAME=="Lumame Town"
...
replace EA_NAME="037-01" if EA_NAME=="NA" & RK_NAME=="Sisa Yekerikeha Den" 
...
replace EA_NAME="002-05" if EA_NAME=="002-02" & RK_NAME=="Alaga Dore"  
*In Afar there are a few kebeles with just woreda codes
replace EA_NAME="012-01" if EA_NAME=="NA" & RK_NAME=="Asayiten"
replace EA_NAME="013-01" if EA_NAME=="NA" & RK_NAME=="Hayiten"
*These seem to be zurias around the towns. I gave them unique but random numbers
replace EA_NAME="015-01" if EA_NAME=="NA" & RK_NAME=="Melka Sedi"
replace EA_NAME="016-01" if EA_NAME=="NA" & RK_NAME=="Andido"
replace EA_NAME="016-01" if EA_NAME=="NA" & RK_NAME=="Mankush Geter"
replace EA_NAME="011-01" if EA_NAME=="001-01" & RK_NAME=="Sekele"
replace EA_NAME="002-04" if EA_NAME=="Soge" & RK_NAME=="Belo Jiganifodo"
replace EA_NAME="025-01" if EA_NAME=="NA" & RK_NAME=="Sefi Yegileseb Yeirisha Meret"
replace EA_NAME="024-01" if EA_NAME=="Manibu" & RK_NAME=="Manibuk"
replace EA_NAME="023-03" if EA_NAME=="023-02" & RK_NAME=="Wanga giten" & y_coord < 1036000
*This is a blank value in Hudene Woreda, I assigned it a random value but mapping would check where it is
replace EA_NAME="018-01" if EA_NAME=="NA" & W_NAME=="Hundene" & x_coord < 838000
*Back to simple corrections
replace T_CODE=3 if W_NAME=="Semen Mekele_Mekele" & RK_NAME=="Mekele Town"
...
replace EA_NAME="003-07" if EA_NAME=="003-03" & RK_NAME=="Gogakemese"  
...
replace EA_NAME="020-01" if EA_NAME=="NA" & RK_NAME=="Farm Land" & W_NAME=="Metema"
replace EA_NAME="021-01" if EA_NAME=="NA" & RK_NAME=="Uncovered by Field Work" & W_NAME=="Metema"
replace EA_NAME="015-03" if EA_NAME=="013-03" & RK_NAME=="May Negad"
replace EA_NAME="024-01" if EA_NAME=="NA" & RK_NAME=="Lafito Yemenigist Den"
...
# run through stata
eas_data_out = stata( source_code, data.in = eas_data, data.out = T)


# error checking some differences in x y coords etc
source_code = 'replace kebele_code_new="98" if RK_CODE_length==7 

list if EA_NAME=="023-02" & RK_NAME=="Wanga giten" '
# head(eas_data[grepl('Lafito Yemenigist Den',eas_data$RK_NAME)  ,])
...
*The 401s of EA_CODE replace with 1 instead of missing 
...
*because one was a duplicate in Afdem Special EA, changed to 90, I didnt get this too well maybe I am tired
...
# run through stata
eas_data_out_codes = stata(source_code , data.in = eas_data_out, data.out = T)



# Put data back into polygons  --------------------------------------------

eas@data = eas_data_out_codes
eas = eas[eas$ea_dup ==0,]

drops <- c("ea_dup","RK_CODE_length",'kebele_code_new','EA_code_new','RK_Code1')
eas@data = eas@data[ , !(names(eas@data) %in% drops)]

head(eas@data)

setwd('C:/Users/mmann/Desktop/AgSS EAS')
writeOGR(eas,'EnumerationAreasSIN_codes.geojson',layer='EnumerationAreasSIN_codes',driver ='GeoJSON')
